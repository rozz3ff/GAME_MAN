...
async function sendScoreToServer(score) {
    const response = await fetch('/api/score', {
        ...
        body: JSON.stringify({ Score_number: score }) // Отправляем актуальный счёт
    });
...
function resetGame() {
    // Отправляем текущий счёт на сервер перед его обнулением
    sendScoreToServer(score);

    // Остальной код для сброса игры
    gameStarted = false;
    ...
    // Обнуляем счёт только после того, как он отправлен на сервер
    score = 0;
